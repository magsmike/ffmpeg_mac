...
##Define Variables
ffmpeg="$HOME/ffmpeg"
download="${ffmpeg}/ffmpeg_sources"
apps="${ffmpeg}/bin"
build="${ffmpeg}/ffmpeg_build"
MAKEFLAGS="-j2"
...
echo "Creating Help2Man folder, downloading, compiling, then deleting..."
  cd ${download} && \
    ...
    PATH="${apps}:$PATH" ./configure --prefix=${build} --bindir=${apps} && \
    make_install && \
  #rm -rf ${DIR}/help2man && \
echo
echo "Creating Libtool folder, downloading, compiling, then deleting..."
  cd ${download} && \
    ...
    PATH="${apps}:$PATH" ./configure --prefix=${build} --bindir=${apps} --disable-dependency-tracking && \
    make_install && \
    #rm -rf ${DIR}/libtool && \
echo
echo "Creating Pkg-Config folder, downloading, compiling, then deleting..."
  cd ${download} && \
    ...
    PATH="${apps}:$PATH" ./configure --prefix=${build} --bindir=${apps} --with-internal-glib --disable-dependency-tracking && \
    make_install && \
...
echo "Creating PERL folder, downloading, compiling, then deleting..."
  cd ${download} && \
    ...
    PATH="${apps}:$PATH" ./Configure -de -Dprefix=/usr/local/ && \
    make_install && \
    #rm -rf ${DIR}/perl && \
echo
